...
// Main SVGs needed for the POS Top Navbar
...
export default function CustomerLayout({ children }) {
  ...
  // Helper to determine styling based on current active route for horizontal nav
  const getNavLinkStyle = (routeName) => ({
    backgroundColor: currentRoute === routeName ? '#D2D1EA' : 'transparent',
    color: currentRoute === routeName ? '#1E1E1E' : '#F5F7FF',
    border: currentRoute === routeName ? '1px solid black' : '1px solid transparent',
  });

  return (
    <div className="d-flex flex-column" style={{ minHeight: '100vh', backgroundColor: '#F5F4FF', fontFamily: "'Poppins', sans-serif" }}>
      
      {/* Top Navbar for POS System */}
      <header className="d-flex justify-content-between align-items-center px-4 shadow-sm" style={{ height: '70px', backgroundColor: '#7978E9', zIndex: 100 }}>
        
        {/* Left Side: Logo and Navigation */}
        <div className="d-flex align-items-center gap-4">
          <div className="d-flex align-items-center gap-2">
            <span className="fw-bold text-white" style={{ fontSize: '1.2rem', letterSpacing: '1px' }}>
              TUNGAL'S FLOWER SHOP
            </span>
            <span className="badge bg-light text-dark d-none d-md-block ms-2 border">POS</span>
          </div>

          <nav className="d-flex gap-2 ms-4">
            <Link 
        ...
        <div className="d-flex align-items-center gap-4">
          <Link href={route('customer.profile')} className="d-flex align-items-center gap-2 text-decoration-none">
            <img
            ...
            />
            <div className="d-flex flex-column text-white d-none d-md-flex">
              <span className="fw-bold" style={{ fontSize: '14px', lineHeight: '1' }}>
                {auth?.user ? `${auth.user.firstname} ${auth.user.lastname}` : 'Cashier'}
              </span>
              <span style={{ fontSize: '11px', color: '#EBEAEE' }}>Active Register</span>
            </div>
          </Link>
          ...
          <Link 
            ...
            method="post" 
            as="button" 
            className="btn btn-sm d-flex align-items-center gap-2 fw-bold text-white shadow-sm"
            style={{ backgroundColor: '#D84B51', border: '1px solid black', borderRadius: '8px', padding: '8px 16px' }}
          >
            <LogoutIcon /> Log Out
          </Link>
        </div>
      ...
      {/* Fullscreen Main Content Area (Removed marginLeft constraint) */}
